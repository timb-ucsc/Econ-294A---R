...
cat("\n\nThis code does the following:\n
      ...
      3) creates a mean-departure-delay variable and a total flights variable
...
day_delay <- flights.df %>%
  dplyr::filter(!is.na(dep_delay), n() > 10) %>%
  group_by(date) %>%
  summarize(
...
day_delay <- day_delay %>%
  mutate(diff.delay = delay - lag(delay, 1, order_by=date, na.pad = TRUE)) %>%
...
hour_delay <- flights.df %>%
  dplyr::filter(!is.na(dep_delay), n() > 10) %>%
  group_by(date, hour) %>%
  summarise(
    dep_delays = mean(dep_delay),
    total = n()
  )

df.10 <- left_join(hour_delay, weather.df, by=c("date"="date", "hour"="hour"))

df.10 <- df.10 %>%
  group_by(conditions) %>%
  summarise(
    delay = mean(dep_delays, na.rm = TRUE),
    total = n()
  )
...
df.11a <- df %>%
  gather(subject, value, subject1:subject2) %>%
